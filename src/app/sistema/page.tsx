...
import { LoginPageVPJS } from "@/components/talon/LoginPageVPJS";
import { Point, LocalPolygon, POLYGON_COLORS } from "@/lib/talon/types";

// Import MapView dynamically to avoid SSR issues with Leaflet
const MapView = dynamic(() => import("@/components/talon/MapView"), {
  ssr: false,
  loading: () => (
...
const AnimalHistoryDialog = dynamic(() => import("@/components/talon/AnimalHistoryDialog").then(mod => ({ default: mod.AnimalHistoryDialog })), {
  ssr: false,
});

// Import AlertSettingsDialog dynamically
const AlertSettingsDialog = dynamic(() => import("@/components/talon/AlertSettingsDialog").then(mod => ({ default: mod.AlertSettingsDialog })), {
  ssr: false,
});

// Função para sincronizar polígonos com Prisma (para sistema de alertas)
// Usa sync completo: deleta todos os antigos e cria os novos de uma vez
async function syncPolygonsWithPrisma(firebaseUid: string, polygons: LocalPolygon[]) {
  try {
    // Enviar lista completa de polígonos para sync (substitui todos)
    const polygonsData = polygons.map(p => ({
      name: p.nome,
      color: p.cor,
      vertices: p.vertices, // Array de {latitude, longitude}
    }));

    const response = await fetch('/api/polygons/alerts', {
      method: 'POST',
    ...
    console.log('🔥 Polígonos sincronizados com Prisma:', result.synced || 0);
  } catch (error) {
      ...
      if (polygonsJson !== lastPolygonsJsonRef.current) {
        ...
        if (polygonsVPJS.length > 0) {
          ...
          queueMicrotask(() => {
            ...
            // Sincronizar polígonos com Prisma IMEDIATAMENTE ao carregar do Firebase
      // Isso garante que polígonos deletados sejam removidos do Prisma
      if (userVPJS?.uidVPJS) {
        const loadedForSync = polygonsVPJS.length > 0
          ? polygonsVPJS.map((p) => ({
              id: p.idVPJS,
              nome: p.nomeVPJS,
              cor: p.corVPJS,
          ...
          : [];
        syncPolygonsWithPrisma(userVPJS.uidVPJS, loadedForSync);
      }
